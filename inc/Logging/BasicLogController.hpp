...
{
public:
  BasicLogController( std::string log_name, std::string log_path ) : m_log_name( log_name ), m_log_path( log_path )
  {
...
private:
  ...
  std::shared_ptr<spdlog::logger> m_logger{ std::make_shared<spdlog::logger>( spdlog::logger(
      m_log_name,
      {
