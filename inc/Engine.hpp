        ...
        case Cmp::GameState::State::PLAYING: {
          ...
          for (auto [_ent, _sys] :
            ...
            if (_sys.collisions_enabled)
              m_collision_sys->check_collision();
...
private:
  ...
  std::unique_ptr<Sys::PlayerSystem> m_player_sys =
      std::make_unique<Sys::PlayerSystem>(m_reg);
  std::shared_ptr<Sys::NpcSystem> m_npc_sys =
      std::make_shared<Sys::NpcSystem>(m_reg);
  ...
  std::unique_ptr<Sys::FloodSystem> m_flood_sys =
      std::make_unique<Sys::FloodSystem>(m_reg, 4.f);
  ...
  void process_action_queue() {
    if (m_event_handler.m_action_queue.empty())
      return;

    if (m_event_handler.m_action_queue.front() ==
        InputEventHandler::GameActions::DROP_BOMB) {
      if (not m_event_handler.m_action_queue.empty())
  ...
  void bootstrap() {
    ...
    m_gamestate_updates.on_update<Cmp::GameState>()
    ...
    m_event_handler.m_gamestate_updates.on_update<Cmp::GameState>()
        .on_construct<Cmp::GameState>();
  ...
  void setup() {
    ...
    m_render_sys->m_system_updates.on_update<Cmp::System>()
    ...
    m_render_sys->m_position_updates.on_update<Cmp::Position>()
        .on_construct<Cmp::Position>();

    m_render_sys->m_flood_updates.bind(*m_reg);
    m_render_sys->m_flood_updates.on_update<Cmp::WaterLevel>()
        .on_construct<Cmp::WaterLevel>();

    // Register the RenderSystem's pool for Obstacle comnponent updates
    m_render_sys->m_position_updates.bind(*m_reg);
    m_render_sys->m_position_updates.on_update<Cmp::Obstacle>()
    ...
    Sys::ProcGen::CellAutomataSystem cellauto_parser{m_reg,
  ...
  void queueinfo() {
    SPDLOG_INFO("{} action events pending",
                m_event_handler.m_action_queue.size());
  }
