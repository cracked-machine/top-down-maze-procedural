...
struct Movement {
  ...
  float acceleration_rate =
      DEFAULT_ACCELERATION_RATE; // Acceleration in pixels per second squared
  float deceleration_rate =
      DEFAULT_DECELERATION_RATE; // Deceleration when no input in pixels per
                                 // second squared
  float friction_coefficient =
      0.02f; // Base friction coefficient when colliding
  float friction_falloff =
      0.5f;                  // How quickly friction decreases with speed (0-1)
