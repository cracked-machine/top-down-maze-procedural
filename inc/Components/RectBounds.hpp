...
public:
  RectBounds(sf::Vector2f position, sf::Vector2f size, float ScaleFactor)
    ...
    m_bounds.position =
        position - sf::Vector2f{Sprites::SpriteFactory::DEFAULT_SPRITE_SIZE} *
                       POSITION_OFFSET;
  }
  ...
  void size(sf::Vector2f new_size) {
    m_bounds.size = new_size * m_scale_factor;
  }
  sf::Vector2f position() { return m_bounds.position; }
  sf::Vector2f size() { return m_bounds.size; }
  std::optional<sf::FloatRect>
  findIntersection(const sf::FloatRect &rectangle) {
