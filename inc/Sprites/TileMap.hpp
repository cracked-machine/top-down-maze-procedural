...
public:
  TileMap() {
    ...
    for (int x = 0; x < 200; x++)
      for (int y = 0; y < 98; y++)
        m_floortile_choices.push_back(
            FLOOR_TILE_POOL[m_floortile_picker.gen()]);

    if (!load(m_tile_file.string(), {16, 16}, m_floortile_choices.data(), 200,
              98)) {
  ...
  bool load(const std::filesystem::path &tileset, sf::Vector2u tileSize,
            const unsigned int *tiles, unsigned int width,
            unsigned int height) {
...
private:
  ...
  const std::vector<unsigned int> FLOOR_TILE_POOL{
      // res/Pixel Lands Dungeons/objects.png
      48, 48, 48, 48, 48, 48, 49};

  Cmp::Random m_floortile_picker{0,
                                 static_cast<int>(FLOOR_TILE_POOL.size() - 1)};

  std::vector<uint32_t> m_floortile_choices;

  std::filesystem::path m_tile_file{
