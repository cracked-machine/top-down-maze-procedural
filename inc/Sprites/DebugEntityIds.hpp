#ifndef __SPRITES_DEBUG_ENTITY_IDS_HPP__
...
{
public:
    DebugEntityIds(const sf::Font& font)
        : m_font(font)
    {
    ...
    void addEntity(uint32_t entity_id, const sf::Vector2f& position)
    {
        // Use larger font size (12-16 pixels minimum for readability)
        sf::Text text(m_font, std::to_string(entity_id), 9);
        text.setFillColor(sf::Color::Black); // White text is more visible
        text.setRotation(sf::degrees(45.f));
        // text.setOutlineColor(sf::Color::Black); // Add black outline for contrast
...
#endif // __SPRITES_DEBUG_ENTITY_IDS_HPP__
