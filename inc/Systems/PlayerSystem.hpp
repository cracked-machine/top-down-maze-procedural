...
public:
  ...
  void update(sf::Time deltaTime) {
    ...
    for (auto [entity, pc_cmp, pos_cmp, move_cmp, dir_cmp, pc_bounds] :
         m_reg
             ->view<Cmp::PlayableCharacter, Cmp::Position, Cmp::Movement,
                    Cmp::Direction, Cmp::PCDetectionBounds>()
             .each()) {
      ...
      if (dir_cmp != sf::Vector2f(0.0f, 0.0f)) {
        ...
        if (move_cmp.velocity != sf::Vector2f(0.0f, 0.0f)) {
          move_cmp.acceleration =
              -move_cmp.velocity.normalized() * move_cmp.deceleration_rate;
        } else {
      ...
      // speed
        ...
        move_cmp.velocity = (move_cmp.velocity / move_cmp.velocity.length()) *
                            move_cmp.max_speed;
      }
