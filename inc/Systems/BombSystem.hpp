...
public:
  ...
  void suspend() {
    ...
    for (auto [_pc_entt, armed] : player_collision_view.each()) {
      if (armed.m_fuse_delay_clock.isRunning())
        armed.m_fuse_delay_clock.stop();
      if (armed.m_warning_delay_clock.isRunning())
        armed.m_warning_delay_clock.stop();
    }
  }
  void resume() {
    auto player_collision_view = m_reg->view<Cmp::Armed>();
    for (auto [_pc_entt, armed] : player_collision_view.each()) {
      if (not armed.m_fuse_delay_clock.isRunning())
        armed.m_fuse_delay_clock.start();
      if (not armed.m_warning_delay_clock.isRunning())
        armed.m_warning_delay_clock.start();
    }
  }
  ...
  void arm_occupied_location() {
    ...
    for (auto [pc_entity, pc_cmp, pc_pos_cmp, movement_cmp] :
         player_collision_view.each()) {
      if (pc_cmp.has_active_bomb)
      ...
      auto obstacle_collision_view = m_reg->view<Cmp::Obstacle, Cmp::Position>(
          entt::exclude<typename Cmp::Armed>);
      for (auto [obstacle_entity, obstacle_cmp, obstacle_pos_cmp] :
        ...
        if (player_hitbox.findIntersection(obstacle_hitbox)) {
          ...
          if (pc_cmp.m_bombdeploycooldowntimer.getElapsedTime() >=
              pc_cmp.m_bombdeploydelay) {
            ...
            pc_cmp.bomb_inventory = (pc_cmp.bomb_inventory > 0)
            ...
            movement_cmp.velocity =
                sf::Vector2f(current_velocity.x * blend_factor +
                                 original_velocity.x * (1.0f - blend_factor),
                             current_velocity.y * blend_factor +
                                 original_velocity.y * (1.0f - blend_factor));
            ...
            if (movement_cmp.velocity.length() > max_restore_speed) {
              movement_cmp.velocity =
                  (movement_cmp.velocity / movement_cmp.velocity.length()) *
                  max_restore_speed;
            }
          }
        }
  ...
  void place_concentric_bomb_pattern(entt::entity &epicenter_entity,
                                     const int BLAST_RADIUS) {
    // arm the current tile (center of the bomb)
    m_reg->emplace_or_replace<Cmp::Armed>(epicenter_entity, sf::seconds(3),
                                          sf::Time::Zero, true, sf::Color::Blue,
                                          -1);
    ...
    m_reg->emplace_or_replace<Cmp::Armed>(
        epicenter_entity, sf::seconds(base_delay), sf::Time::Zero, true,
    ...
    for (int layer = 1; layer <= BLAST_RADIUS; layer++) {
      ...
      for (auto [obstacle_entity, obstacle_cmp, obstacle_pos_cmp] :
        ...
        if (obstacle_entity == epicenter_entity ||
            m_reg->any_of<Cmp::Armed>(obstacle_entity))
          continue;
      ...
      for (const auto &[entity, pos] : layer_entities) {
        sf::Color color =
            sf::Color(255, 100 + (sequence_counter * 10) % 155, 255, 64);

        m_reg->emplace_or_replace<Cmp::Armed>(
            entity,
            sf::seconds(base_delay + (sequence_counter * fuse_delay_increment)),
            sf::seconds((sequence_counter * warning_delay_increment)), false,
  ...
  void update() {
    auto armed_view =
        m_reg
            ->view<Cmp::Armed, Cmp::Obstacle, Cmp::Neighbours, Cmp::Position>();
    for (auto [_entt, _armed_cmp, _obstacle_cmp, _neighbours_cmp,
               _ob_pos_comp] : armed_view.each()) {
      if (_armed_cmp.getElapsedFuseTime() < _armed_cmp.m_fuse_delay)
        continue;
      if (_obstacle_cmp.m_enabled && not _obstacle_cmp.m_broken) {
        ...
        if (_obstacle_cmp.m_type == Sprites::SpriteFactory::Type::POT) {
          auto random_selected_loot_metadata =
              m_sprite_factory->get_random_metadata(
                  std::vector<Sprites::SpriteFactory::Type>{
      ...
      for (auto [npc_entt, npc_cmp, npc_pos_cmp] :
           m_reg->view<Cmp::NPC, Cmp::Position>().each()) {
...
private:
  ...
  int player_damage =
      10; // Amount of damage to deal to the player when hit by explosion
