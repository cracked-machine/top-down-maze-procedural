...
public:
  ...
  void suspend() {
    ...
    for (auto [_pc_entt, player] : player_collision_view.each()) {
      if (player.m_bombdeploycooldowntimer.isRunning())
        player.m_bombdeploycooldowntimer.stop();
    }
  }
  void resume() {
    auto player_collision_view = m_reg->view<Cmp::PlayableCharacter>();
  ...
  void check_bones_reanimation() {
    auto player_collision_view =
        m_reg->view<Cmp::PlayableCharacter, Cmp::Position>();
    ...
    for (auto [_pc_entt, _pc, _pc_pos] : player_collision_view.each()) {
      ...
      for (auto [_obstacle_entt, _obstacle, _obstacle_pos] :
           obstacle_collision_view.each()) {
        if (_obstacle.m_type != Sprites::SpriteFactory::Type::BONES ||
            not _obstacle.m_enabled || not _obstacle.m_visible)
          continue;

        auto obstacle_hitbox = get_hitbox(_obstacle_pos);
        obstacle_hitbox.size *= 2.f;
        obstacle_hitbox.position.x -=
            Sprites::SpriteFactory::DEFAULT_SPRITE_SIZE.x * 0.5f;
        obstacle_hitbox.position.y -=
            Sprites::SpriteFactory::DEFAULT_SPRITE_SIZE.y * 0.5f;
        ...
        if (player_hitbox.findIntersection(obstacle_hitbox)) {
          ...
          m_reg->emplace_or_replace<Cmp::Obstacle>(
              _obstacle_entt, Sprites::SpriteFactory::Type::BONES, 0, false,
              false);
  ...
  void check_player_to_npc_collision() {
    auto player_collision_view =
        m_reg->view<Cmp::PlayableCharacter, Cmp::Position, Cmp::Direction,
                    Cmp::Movement>();
    auto npc_collision_view = m_reg->view<Cmp::NPC, Cmp::Position>();
    for (auto [_pc_entt, _pc, _pc_pos, _direction, _movement] :
      ...
      for (auto [_npc_entt, _npc, _npc_pos] : npc_collision_view.each()) {
        ...
        if (player_hitbox.findIntersection(npc_hitbox)) {
          if (_npc.m_damage_cooldown.getElapsedTime() < _npc.DAMAGE_DELAY)
            continue;
          _pc.health -= 10;
          ...
          if (_direction.x != 0.f || _direction.y != 0.f) {
            ...
            sf::Vector2f push_dir = {_pc_pos.x - _npc_pos.x,
                                     _pc_pos.y - _npc_pos.y};
  ...
  void check_loot_collision() {
    ...
    auto player_collision_view =
        m_reg->view<Cmp::PlayableCharacter, Cmp::Position, Cmp::Movement>();
      ...
      for (auto [_loot_entt, _loot, _loot_pos] : loot_collision_view.each()) {
        ...
        if (player_hitbox.findIntersection(loot_hitbox)) {
          ...
          loot_effects.push_back(
              {_loot_entt, _loot.m_type, _pc_entt, _movement.velocity});
        }
      }
    }
  ...
  void update_obstacle_distances() {
    auto player_view = m_reg->view<Cmp::PlayableCharacter, Cmp::Position,
                                   Cmp::Movement, Cmp::PCDetectionBounds>();
    for (auto [_pc_entt, _pc, _pc_pos, _movement, pc_detection_bounds] :
         player_view.each()) {

      auto obstacle_view = m_reg->view<Cmp::Obstacle, Cmp::Position>();
      for (auto [_ob_entt, _ob, _ob_pos] : obstacle_view.each()) {
        // while we are here calculate the obstacle/player distance for any
        // traversable obstacles
        if (not _ob.m_enabled &&
            pc_detection_bounds.findIntersection(get_hitbox(_ob_pos))) {
  ...
  void check_collision() {
    const float PUSH_FACTOR =
        1.1f; // Push slightly more than minimum to avoid floating point issues
    auto player_view =
        m_reg->view<Cmp::PlayableCharacter, Cmp::Position, Cmp::Movement>();
    for (auto [_pc_entt, _pc, _pc_pos, _movement] : player_view.each()) {
      ...
      for (auto [_ob_entt, _ob, _ob_pos] : obstacle_view.each()) {
        ...
        // otherwise we are not interested in collision detection on traversable
        // obstacles
        if (not _ob.m_enabled) {
          continue;
        }
        ...
        if (stuck_loop >
            5) // Reduced threshold, but we'll be smarter about resolution
        {
        ...
        auto minXDist =
            (player_floatrect.size.x / 2.0f) + (brick_floatRect.size.x / 2.0f);
        auto minYDist =
            (player_floatrect.size.y / 2.0f) + (brick_floatRect.size.y / 2.0f);

        // Calculate signed penetration depths
        float depthX =
            (diffX > 0 ? 1.0f : -1.0f) * (minXDist - std::abs(diffX));
        float depthY =
            (diffY > 0 ? 1.0f : -1.0f) * (minYDist - std::abs(diffY));
        ...
        if (std::abs(depthX) < std::abs(depthY)) {
          ...
          float speed_ratio =
              std::abs(_movement.velocity.y) / _movement.max_speed;
          float dynamic_friction =
              _movement.friction_coefficient *
          ...
          // Calculate speed-based friction coefficient
          float speed_ratio =
              std::abs(_movement.velocity.x) / _movement.max_speed;
          float dynamic_friction =
              _movement.friction_coefficient *
              (1.0f - (_movement.friction_falloff * speed_ratio));
        ...
        SPDLOG_DEBUG("Collision resolved - new pos: {},{}", _pc_pos.x,
                     _pc_pos.y);
      }
    }
  }
...
private:
  ...
  sf::FloatRect m_end_zone{{DISPLAY_SIZE.x * 1.f, 0},
