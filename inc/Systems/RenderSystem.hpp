...
public:
  RenderSystem(std::shared_ptr<entt::basic_registry<entt::entity>> reg,
    ...
    m_local_view =
        sf::View({LOCAL_MAP_VIEW_SIZE.x * 0.5f, LOCAL_MAP_VIEW_SIZE.y * 0.5f},
                 LOCAL_MAP_VIEW_SIZE);
    m_local_view.setViewport(sf::FloatRect({0.f, 0.f}, {1.f, 1.f}));

    // init minimap view dimensions
    m_minimap_view =
        sf::View({MINI_MAP_VIEW_SIZE.x * 0.5f, MINI_MAP_VIEW_SIZE.y * 0.5f},
                 MINI_MAP_VIEW_SIZE);
    ...
    if (not m_detonation_ms) {
      SPDLOG_CRITICAL(
          "Unable to get DETONATION multisprite from SpriteFactory");
      std::get_terminate();
    }
    if (not m_wall_ms) {
    ...
    if (not m_extra_health_ms) {
      SPDLOG_CRITICAL(
          "Unable to get EXTRA_HEALTH multisprite from SpriteFactory");
      std::get_terminate();
    }
    if (not m_extra_bombs_ms) {
      SPDLOG_CRITICAL(
          "Unable to get EXTRA_BOMBS multisprite from SpriteFactory");
      std::get_terminate();
    }
    if (not m_infinite_bombs_ms) {
      SPDLOG_CRITICAL(
          "Unable to get INFINI_BOMBS multisprite from SpriteFactory");
      std::get_terminate();
    }
    if (not m_chain_bombs_ms) {
      SPDLOG_CRITICAL(
          "Unable to get CHAIN_BOMBS multisprite from SpriteFactory");
      std::get_terminate();
    }
    if (not m_lower_water_ms) {
      SPDLOG_CRITICAL(
          "Unable to get LOWER_WATER multisprite from SpriteFactory");
      std::get_terminate();
    }
  ...
  void render_game() {
    ...
    m_window->clear();
    {
      ...
      {
        render_floormap(
            {0, MAP_GRID_OFFSET.y * m_sprite_factory->DEFAULT_SPRITE_SIZE.y});
        render_obstacles();
        render_armed();
        render_loot();
        render_walls();
        ...
        for (auto [_ent, _sys] : m_system_updates.view<Cmp::System>().each()) {
          if (_sys.player_stuck) {
            m_local_view.setCenter(
                {LOCAL_MAP_VIEW_SIZE.x * 0.5f, DISPLAY_SIZE.y * 0.5f});
            _sys.player_stuck = false;
          } else {
            for (auto [entity, _pc, _pos] :
                 m_position_updates
                     .view<Cmp::PlayableCharacter, Cmp::Position>()
                     .each()) {
      ...
      {
        auto minimap_border =
            sf::RectangleShape({MINI_MAP_VIEW_SIZE.x, MINI_MAP_VIEW_SIZE.y});
        ...
        for (auto [_entt, water_level] :
             m_reg->view<Cmp::WaterLevel>().each()) {
          render_water_level_meter_overlay(water_level.m_level, {40.f, 70.f},
                                           {200.f, 20.f});
        }
  ...
  std::shared_ptr<Sprites::SpriteFactory> m_sprite_factory =
      std::make_shared<Sprites::SpriteFactory>();
...
private:
  ...
  void render_obstacles() {
    ...
    std::vector<std::pair<sf::Vector2f, Sprites::SpriteFactory::Type>>
        disabledPositions;
    ...
    for (auto [entity, _ob, _pos, _ob_nb_list] :
         m_position_updates
             .view<Cmp::Obstacle, Cmp::Position, Cmp::Neighbours>()
             .each()) {
    ...
    //     sf::RectangleShape
    //     temp_square(sf::Vector2f{m_sprite_factory->DEFAULT_SPRITE_SIZE});
  ...
  void render_armed() {
    ...
    auto all_armed_obstacles_view =
        m_reg->view<Cmp::Obstacle, Cmp::Armed, Cmp::Position>();
    for (auto [entity, obstacle_cmp, armed_cmp, pos_cmp] :
         all_armed_obstacles_view.each()) {
      ...
      sf::RectangleShape temp_square(
          sf::Vector2f{m_sprite_factory->DEFAULT_SPRITE_SIZE});
  ...
  void render_loot() {
    auto loot_view =
        m_position_updates.view<Cmp::Obstacle, Cmp::Loot, Cmp::Position>();
  ...
  void render_walls() {
    ...
    for (auto [entity, _ob, _pos] :
         m_position_updates
             .view<Cmp::Obstacle, Cmp::Position>(entt::exclude<Cmp::Neighbours>)
             .each()) {
  ...
  void render_npc() {
    for (auto [entity, npc, pos, npc_scan_bounds, direction] :
         m_position_updates
             .view<Cmp::NPC, Cmp::Position, Cmp::NPCScanBounds,
                   Cmp::Direction>()
             .each()) {
      // flip and x-axis offset the sprite depending on the direction
      if (direction.x > 0) {
  ...
  void render_player_distances_on_obstacles() {
    if (!m_show_path_distances)
      return;
    auto obstacle_view =
        m_reg->view<Cmp::Obstacle, Cmp::Position, Cmp::PlayerDistance>();
    for (auto [_ob_entt, _ob, _ob_pos, _player_distance] :
         obstacle_view.each()) {
      sf::Text distance_text(m_font, "", 10);
  ...
  bool isInView(const sf::View &view, const sf::Vector2f &position,
  ...
  void update_view_center(sf::View &view, Cmp::Position &player_pos) {
    ...
    float smoothFactor =
        0.1f; // Adjust this value to change how quickly the camera follows
  ...
  void render_entt_distance_set_overlay(sf::Vector2f pos) {
    ...
    for (auto [e, distance_map] : entt_distance_map_view.each()) {
      ...
      if (distance_map.empty()) {
        ...
        distance_text.setPosition(pos +
                                  sf::Vector2f{0, entt_distance_set * 30.f});
  ...
  void render_health_overlay(float health_value, sf::Vector2f pos,
                             sf::Vector2f size) {
    // text
    ...
    auto healthbar =
        sf::RectangleShape({((size.x / 100) * health_value), size.y});
  ...
  void render_water_level_meter_overlay(float water_level, sf::Vector2f pos,
                                        sf::Vector2f size) {
    // text
    ...
    // position
    float meter_meter_level =
        size.x - ((size.x / DISPLAY_SIZE.y) * water_level);
...
private:
  std::unique_ptr<sf::RenderWindow> m_window =
      std::make_unique<sf::RenderWindow>(sf::VideoMode(DISPLAY_SIZE),
                                         "ProceduralMaze");
  ...
  std::optional<Sprites::MultiSprite> m_rock_ms =
      m_sprite_factory->get_multisprite_by_type(
          Sprites::SpriteFactory::Type::ROCK);
  std::optional<Sprites::MultiSprite> m_pot_ms =
      m_sprite_factory->get_multisprite_by_type(
          Sprites::SpriteFactory::Type::POT);
  ...
  const sf::Vector2f MINI_MAP_VIEW_SIZE{DISPLAY_SIZE.x * 0.25f,
