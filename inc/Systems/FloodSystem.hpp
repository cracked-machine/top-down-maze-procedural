...
private:
  static constexpr float FIXED_TIMESTEP =
      1.0f / 30.0f; // Reduce to 30 FPS to decrease CPU load
  static constexpr float DAMAGE_COOLDOWN =
      1.0f; // 1 second between damage applications
...
private:
  void updateFlood(float dt) {
    ...
    for (auto [_, water_level] : water_view.each()) {
      for (auto [player_entity, player_char, position, move_cmp, dir_cmp] :
           player_view.each()) {
        if (water_level.m_level <=
            position.y) // Water drowns player when water level is at or above
                        // player position
        {
          if (m_abovewater_sound_player.getStatus() ==
          ...
          move_cmp.acceleration_rate =
              move_cmp.DEFAULT_ACCELERATION_RATE * 0.5f;
          move_cmp.deceleration_rate =
              move_cmp.DEFAULT_DECELERATION_RATE * 0.15f;
