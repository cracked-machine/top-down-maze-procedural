...
public:
  ...
  void lerp_movement(sf::Time dt) {
    auto view =
        m_reg->view<Cmp::Position, Cmp::LerpPosition, Cmp::NPCScanBounds>();
    ...
    for (auto [entity, pos, target, npc_scan_bounds] : view.each()) {
      ...
      if (target.m_lerp_factor >= 1.0f) {
        ...
        pos.x = std::lerp(target.m_start.x, target.m_target.x,
                          target.m_lerp_factor);
        pos.y = std::lerp(target.m_start.y, target.m_target.y,
                          target.m_lerp_factor);
      }
