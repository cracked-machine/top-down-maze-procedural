...
public:
  CellAutomataSystem(std::shared_ptr<entt::basic_registry<entt::entity>> reg,
  ...
  void iterate(unsigned int iterations) {
    ...
    for (unsigned int i = 0; i < iterations; i++) {
      find_neighbours();
      apply_rules();
      SPDLOG_INFO("#{} took {}ms", i,
                  iteration_timer.restart().asMilliseconds());
    }
...
private:
  ...
  void find_neighbours() {
    ...
    for (auto it = m_random_level->begin(); it != m_random_level->end(); it++) {
      if (!m_reg->valid(entt::entity(*it))) {
        SPDLOG_WARN("Entity {} is not valid! Valid entities are:",
                    entt::to_integral(*it));
      ...
      m_reg->patch<Cmp::Neighbours>(
          entt::entity(*it), [](auto &_nb_update) { _nb_update.clear(); });
      ...
      if (std::prev(it) >= m_random_level->begin()) {
        ...
        if (not left_entt_ob) {
          ...
          assert(false &&
                 ...
                 "neighbour: " &&
                 entt::to_integral(left_entt));
        } else if (left_entt_ob && left_entt_ob->m_enabled &&
                   not has_left_map_edge) {
      ...
      if (std::prev(it, (MAP_GRID_SIZE.y + 1)) >= m_random_level->begin()) {
        ...
        Cmp::Obstacle *down_left_entt_ob =
            m_reg->try_get<Cmp::Obstacle>(down_left_entt);
      ...
      // N - (y + 1)
      if ((std::prev(it, (MAP_GRID_SIZE.y - 1))) >= m_random_level->begin()) {
        auto down_right_entt =
            entt::entity(*std::prev(it, MAP_GRID_SIZE.y - 1));
        Cmp::Obstacle *down_right_entt_ob =
            m_reg->try_get<Cmp::Obstacle>(down_right_entt);
      ...
      if (std::next(it, (MAP_GRID_SIZE.y - 1)) < m_random_level->end()) {
        ...
        Cmp::Obstacle *top_left_entt_ob =
            m_reg->try_get<Cmp::Obstacle>(top_left_entt);
      ...
      if ((MAP_GRID_SIZE.y + 1) < m_random_level->size() &&
        ...
        auto top_right_entt =
            entt::entity(*std::next(it, (MAP_GRID_SIZE.y + 1)));
        Cmp::Obstacle *top_right_entt_ob =
            m_reg->try_get<Cmp::Obstacle>(top_right_entt);
      ...
      if (std::next(it) < m_random_level->end()) {
        ...
        Cmp::Obstacle *right_entt_ob =
            m_reg->try_get<Cmp::Obstacle>(right_entt);
    ...
    SPDLOG_INFO("Processed neighbours for {} entities.",
                m_random_level->size());
  ...
  void apply_rules() {
    ...
    for (auto [_entt, _ob, _pos, _nb] :
         m_reg->view<Cmp::Obstacle, Cmp::Position, Cmp::Neighbours>().each()) {
      if (_ob.m_type == Sprites::SpriteFactory::Type::WALL) {
        continue;
      }
      if (_nb.count() <= 2) {
        _ob.m_enabled = true;
      } else if (_nb.count() > 2 and _nb.count() < 5) {
        _ob.m_enabled = false;
      } else {
        _ob.m_enabled = true;
      }
