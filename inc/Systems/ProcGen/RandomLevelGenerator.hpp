...
public:
    ...
    void gen_border(entt::basic_registry<entt::entity> &reg)
    {
        ...
        {
            // left edge 
            add_border_entity( reg, {0, y} );
            // right edge
            add_border_entity( reg,{static_cast<float>(DISPLAY_SIZE.x) -  Sprites::Brick::WIDTH, y} );
        }

    }

    void add_border_entity(entt::basic_registry<entt::entity> &reg, const sf::Vector2f &pos)
    {
        auto entity = reg.create();
        reg.emplace<Cmp::Position>(entity, pos); 
        auto tile_pick = m_border_tile_choices[m_random_border_tile_picker.gen()];
        reg.emplace<Cmp::Obstacle>(entity, tile_pick, Cmp::Obstacle::Type::BEDROCK, true, true );
    }

    void stats(entt::basic_registry<entt::entity> &reg)
    {
