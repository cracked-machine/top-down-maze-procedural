...
public:
    ...
    for (unsigned int x = 0; x < Sys::BaseSystem::MAP_GRID_SIZE.x; x++) {
      for (unsigned int y = 0; y < Sys::BaseSystem::MAP_GRID_SIZE.y; y++) {
        ...
        m_reg->emplace<Cmp::Position>(
            entity,
            sf::Vector2f{(x * m_sprite_factory->DEFAULT_SPRITE_SIZE.x) +
                             (Sys::BaseSystem::MAP_GRID_OFFSET.x *
                              m_sprite_factory->DEFAULT_SPRITE_SIZE.x),
                         (y * m_sprite_factory->DEFAULT_SPRITE_SIZE.y) +
                             (Sys::BaseSystem::MAP_GRID_OFFSET.y *
                              m_sprite_factory->DEFAULT_SPRITE_SIZE.y)});
        ...
        auto random_obstacle_texture_index =
            obstacle_metadata->pick_random_texture_index();
        m_reg->emplace<Cmp::Obstacle>(entity, obstacle_metadata->get_type(),
                                      random_obstacle_texture_index, true,
  ...
  void gen_border() {
    ...
    bool enabled = true;
    for (float x = 0; x < DISPLAY_SIZE.x;
         x += m_sprite_factory->DEFAULT_SPRITE_SIZE.x) {
      if (x == 0 ||
          x == DISPLAY_SIZE.x - m_sprite_factory->DEFAULT_SPRITE_SIZE.x)
        texture_index = 2;
      else
        texture_index = 1;
      // top edge
      add_border_entity({x, (MAP_GRID_OFFSET.y - 1) *
                                m_sprite_factory->DEFAULT_SPRITE_SIZE.y},
                        texture_index);
      // bottom edge
      add_border_entity({x, MAP_GRID_OFFSET.y +
                                ((MAP_GRID_SIZE.y + 2) *
                                 m_sprite_factory->DEFAULT_SPRITE_SIZE.y) -
                                2},
                        texture_index);
    }
    for (float y = 0; y < DISPLAY_SIZE.y;
         y += m_sprite_factory->DEFAULT_SPRITE_SIZE.y) {
      if (y == 0 || y == DISPLAY_SIZE.y - 1)
        texture_index = 2;
      else if (y ==
               (DISPLAY_SIZE.y / 2.f) - m_sprite_factory->DEFAULT_SPRITE_SIZE.y)
        texture_index = 3;
      ...
      if (y == (DISPLAY_SIZE.y / 2.f)) {
        texture_index = 6;
        enabled = false;
      } // open door exit
      // right edge
      add_border_entity({static_cast<float>(DISPLAY_SIZE.x) -
                             m_sprite_factory->DEFAULT_SPRITE_SIZE.x,
                         y},
  ...
  void add_border_entity(const sf::Vector2f &pos, std::size_t texture_index,
                         bool enabled = true) {
    auto entity = m_reg->create();
    m_reg->emplace<Cmp::Position>(entity, pos);
    auto wall_ms = m_sprite_factory->get_metadata_by_type(
    ...
    m_reg->emplace<Cmp::Obstacle>(entity, Sprites::SpriteFactory::Type::WALL,
                                  texture_index, true, enabled);
    m_reg->emplace<Cmp::Exit>(entity);
  }

  void stats() {
    std::map<std::string, int> results;
    for (auto [entity, _pos, _ob] :
         m_reg->view<Cmp::Position, Cmp::Obstacle>().each()) {
