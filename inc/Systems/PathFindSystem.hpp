...
public:
  ...
  void scanForPlayers(entt::entity npc_entity, entt::entity player_entity) {
    ...
    auto pc_detection_bounds =
        m_reg->try_get<Cmp::PCDetectionBounds>(player_entity);
    if (not npc_scan_bounds || not pc_detection_bounds)
      return;

    // only continue if player is within detection distance
    if (not npc_scan_bounds->findIntersection(
            pc_detection_bounds->getBounds())) {
      ...
      for (auto [obstacle_entity, next_pos, player_distance] :
           obstacle_view.each()) {
      ...
      if (npc_cmp) {
        ...
        if (npc_lerp_pos_cmp && npc_lerp_pos_cmp->m_lerp_factor < 1.0f) {
          ...
          m_reg->emplace_or_replace<Cmp::Direction>(npc_entity,
                                                    distance.normalized());

          // Otherwise set target position for lerp. This will get updated in
          // the main engine loop via LerpSystems
          auto move_candidate_pixel_pos =
              getPixelPosition(nearest_obstacle.second);
          if (not move_candidate_pixel_pos)
            return;
          m_reg->emplace_or_replace<Cmp::LerpPosition>(
              npc_entity, move_candidate_pixel_pos.value(), 0.0f);
        }
      }
    }
  }
