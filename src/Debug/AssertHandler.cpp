    ...
    for (size_t i = 0; i < 25; i++) 
    {
        ...
        if (SymFromAddr(process, stackframe.AddrPC.Offset, &displacement, symbol))
        {
            ...
            if (SymGetLineFromAddr64(process, stackframe.AddrPC.Offset, &lineDisplacement, &line)) 
            {
                ...
                if (hasModuleInfo) 
                {
                    SPDLOG_CRITICAL("    Module: {}", moduleInfo.ModuleName);
                }
            } 
            else 
            {
                // We only have function name
                SPDLOG_CRITICAL("[{:2}] {} (no line info)", 
                    i,
                    symbol->Name);
            }  
        }
        else 
        {
            ...
            SPDLOG_CRITICAL("[{:2}] Unknown function at {:016X}",
                i,
...
#else // For non-Windows platforms, we can use a placeholder or implement a different stack trace mechanism
