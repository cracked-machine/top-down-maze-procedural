...
void stack_trace(void) {
  ...
  for (size_t i = 0; i < 25; i++) {
    ...
    if (SymFromAddr(process, stackframe.AddrPC.Offset, &displacement, symbol)) {
      ...
      bool hasModuleInfo =
          SymGetModuleInfo64(process, stackframe.AddrPC.Offset, &moduleInfo);

      if (SymGetLineFromAddr64(process, stackframe.AddrPC.Offset,
                               &lineDisplacement, &line)) {
        // We have full symbol information
        SPDLOG_CRITICAL("[{:2}] {} in {}:{}", i, symbol->Name, line.FileName,
                        line.LineNumber);
      ...
      SPDLOG_CRITICAL("[{:2}] Unknown function at {:016X}", i,
