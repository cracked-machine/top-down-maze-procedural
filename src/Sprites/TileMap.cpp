...
void TileMap::initialize( entt::registry &registry, const TileMapConfig &config )
{
...
void TileMap::create( entt::registry &registry, sf::Vector2u tile_size, unsigned int width, unsigned int height )
{
  ...
  for ( auto [entity, pos_cmp, reserved_cmp] : grid_view.each() )
  {
    ...
    unsigned int i = static_cast<unsigned int>( pos_cmp.position.x / tile_size.x );
    unsigned int j = static_cast<unsigned int>( pos_cmp.position.y / tile_size.y );
