...
PlantTwigsParticleSprite::PlantTwigsParticleSprite( size_t count )
    : SpriteBase( count ) {};

void PlantTwigsParticleSprite::simulate( sf::Time dt )
{
...
void PlantTwigsParticleSprite::draw( sf::RenderTarget &target, sf::RenderStates states ) const
{
  ...
  for ( const auto &p : m_particles_list )
  {
    ...
    // orient the twig along its direction of travel, so it reads as a rigid stick rather than a blob
    const sf::Angle angle = p.m_velocity.angle();

    const std::array<sf::Vector2f, 4> corners = {
        pos + sf::Vector2f( -half_length, -half_width ).rotatedBy( angle ),
        pos + sf::Vector2f( half_length, -half_width ).rotatedBy( angle ),
        pos + sf::Vector2f( half_length, half_width ).rotatedBy( angle ),
        pos + sf::Vector2f( -half_length, half_width ).rotatedBy( angle ),
    };
