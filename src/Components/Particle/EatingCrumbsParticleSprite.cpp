...
namespace detail
{
void EatingCrumbsParticle::emit()
{
  ...
  // crumbs should always fall away from the mouth, never fly upward — mirror any upward
  // component down rather than letting a North-facing (or edge-of-spread) angle send them up
  if ( m_velocity.y < 0.f ) m_velocity.y = -m_velocity.y;
...
EatingCrumbsParticleSprite::EatingCrumbsParticleSprite( size_t count )
    : SpriteBase( count ) {};

void EatingCrumbsParticleSprite::simulate( sf::Time dt )
{
  for ( auto &p : m_particles_list )
  {
    ...
    p.m_vertex.color.r = 139 + p.m_color_variation;
    p.m_vertex.color.g = 69 + p.m_color_variation;
    p.m_vertex.color.b = 19 + p.m_color_variation;
  }
}

void EatingCrumbsParticleSprite::draw( sf::RenderTarget &target, sf::RenderStates states ) const
{
  ...
  for ( const auto &p : m_particles_list )
  {
    ...
    constexpr size_t kVertexCount = detail::EatingCrumbsParticle::kVertexCount;
