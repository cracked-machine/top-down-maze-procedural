...
PlantLeavesParticleSprite::PlantLeavesParticleSprite( size_t count )
    : SpriteBase( count ) {};

void PlantLeavesParticleSprite::simulate( sf::Time dt )
{
  for ( auto &p : m_particles_list )
  {
    ...
    p.m_vertex.color.g = 130 + p.m_color_variation;
    p.m_vertex.color.b = 60 + p.m_color_variation;
  }
}

void PlantLeavesParticleSprite::draw( sf::RenderTarget &target, sf::RenderStates states ) const
{
  ...
  for ( const auto &p : m_particles_list )
  {
    ...
    constexpr size_t kVertexCount = detail::PlantLeavesParticle::kVertexCount;
