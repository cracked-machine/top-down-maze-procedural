...
void RenderMenuSystem::render_settings_widgets( sf::Time globalDeltaTime )
{
  ...
  ImGui::SliderScalar( "Bomb Inventory", ImGuiDataType_S32, &bomb_inventory.get_value(), &bomb_inventory.get_min_value(),
                       &bomb_inventory.get_max_value(), "%d" );

  auto &blast_radius = get_persistent_component<Cmp::Persistent::BlastRadius>();
  ImGui::SliderScalar( "Blast Radius", ImGuiDataType_S32, &blast_radius.get_value(), &blast_radius.get_min_value(),
                       &blast_radius.get_max_value(), "%d" );

  auto &player_anim_framerate = get_persistent_component<Cmp::Persistent::PlayerAnimFramerate>();
  ImGui::SliderScalar( "Player Animation Framerate", ImGuiDataType_Float, &player_anim_framerate.get_value(),
                       &player_anim_framerate.get_min_value(), &player_anim_framerate.get_max_value(), "%.2f" );

  auto &player_lerp_speed = get_persistent_component<Cmp::Persistent::PlayerLerpSpeed>();
  ImGui::SliderScalar( "Player Lerp Speed", ImGuiDataType_Float, &player_lerp_speed.get_value(), &player_lerp_speed.get_min_value(),
                       &player_lerp_speed.get_max_value(), "%.1f" );

  auto &player_diagonal_lerp_speed_modifier = get_persistent_component<Cmp::Persistent::PlayerDiagonalLerpSpeedModifier>();
  ImGui::SliderScalar( "Player Diagonal Lerp Speed Modifier", ImGuiDataType_Float, &player_diagonal_lerp_speed_modifier.get_value(),
                       &player_diagonal_lerp_speed_modifier.get_min_value(), &player_diagonal_lerp_speed_modifier.get_max_value(),
                       "%.2f" );

  auto &player_shortcut_lerp_speed_modifier = get_persistent_component<Cmp::Persistent::PlayerShortcutLerpSpeedModifier>();
  ImGui::SliderScalar( "Player Shortcut Lerp Speed Modifier", ImGuiDataType_Float, &player_shortcut_lerp_speed_modifier.get_value(),
                       &player_shortcut_lerp_speed_modifier.get_min_value(), &player_shortcut_lerp_speed_modifier.get_max_value(),
                       "%.2f" );

  auto &digging_cooldown = get_persistent_component<Cmp::Persistent::DiggingCooldownThreshold>();
  ImGui::SliderScalar( "Digging Cooldown", ImGuiDataType_Float, &digging_cooldown.get_value(), &digging_cooldown.get_min_value(),
                       &digging_cooldown.get_max_value(), "%.2f seconds" );

  auto &digging_damage_per_hit = get_persistent_component<Cmp::Persistent::DiggingDamagePerHit>();
  ImGui::SliderScalar( "Digging Damage Per Hit", ImGuiDataType_Float, &digging_damage_per_hit.get_value(),
                       &digging_damage_per_hit.get_min_value(), &digging_damage_per_hit.get_max_value(), "%.2f damage" );

  auto &weapon_degrade_per_hit = get_persistent_component<Cmp::Persistent::WeaponDegradePerHit>();
  ImGui::SliderScalar( "Weapon Degrade Per Hit", ImGuiDataType_Float, &weapon_degrade_per_hit.get_value(),
                       &weapon_degrade_per_hit.get_min_value(), &weapon_degrade_per_hit.get_max_value(), "%.2f level" );

  auto &pc_detection_scale = get_persistent_component<Cmp::Persistent::PlayerDetectionScale>();
  ImGui::SliderScalar( "PC Detection Bounding Box Scale Factor", ImGuiDataType_Float, &pc_detection_scale.get_value(),
                       &pc_detection_scale.get_min_value(), &pc_detection_scale.get_max_value(), "%.1f pixels" );

  auto &pc_damage_cooldown = get_persistent_component<Cmp::Persistent::PcDamageDelay>();
  ImGui::SliderScalar( "PC Damage Cooldown", ImGuiDataType_Float, &pc_damage_cooldown.get_value(),
                       &pc_damage_cooldown.get_min_value(), &pc_damage_cooldown.get_max_value(), "%.1f seconds" );

  // Bomb Settings
  ImGui::SeparatorText( "Bomb Settings" );

  auto &bomb_damage = get_persistent_component<Cmp::Persistent::BombDamage>();
  ImGui::SliderScalar( "Bomb Damage", ImGuiDataType_S32, &bomb_damage.get_value(), &bomb_damage.get_min_value(),
                       &bomb_damage.get_max_value(), "%d" );

  auto &fuse_delay = get_persistent_component<Cmp::Persistent::FuseDelay>();
  ImGui::SliderScalar( "Fuse Delay", ImGuiDataType_Float, &fuse_delay.get_value(), &fuse_delay.get_min_value(),
  ...
  ImGui::SliderScalar( "Armed Off Delay", ImGuiDataType_Float, &armed_off_delay.get_value(), &armed_off_delay.get_min_value(),
                       &armed_off_delay.get_max_value(), "%.3f" );

  // Hazard Settings
  ImGui::SeparatorText( "Hazard Settings" );
  auto &corruption_damage = get_persistent_component<Cmp::Persistent::CorruptionDamage>();
  ImGui::SliderScalar( "Corruption Damage", ImGuiDataType_S32, &corruption_damage.get_value(), &corruption_damage.get_min_value(),
                       &corruption_damage.get_max_value(), "%d" );

  auto &wormhole_anim_framerate = get_persistent_component<Cmp::Persistent::WormholeAnimFramerate>();
  ImGui::SliderScalar( "Wormhole Animation Framerate", ImGuiDataType_Float, &wormhole_anim_framerate.get_value(),
  ...
  ImGui::SliderScalar( "Health Bonus", ImGuiDataType_S64, &health_bonus.get_value(), &health_bonus.get_min_value(),
                       &health_bonus.get_max_value(), "%.1f" );
  auto &bomb_bonus = get_persistent_component<Cmp::Persistent::BombBonus>();
  ImGui::SliderScalar( "Bomb Bonus", ImGuiDataType_S64, &bomb_bonus.get_value(), &bomb_bonus.get_min_value(),
                       &bomb_bonus.get_max_value(), "%.1f" );

  // NPC Settings
  ImGui::SeparatorText( "NPC Settings" );

  auto &npc_anim_framerate = get_persistent_component<Cmp::Persistent::NpcSkeleAnimFramerate>();
  ImGui::SliderScalar( "NPC Animation Skeleton Framerate", ImGuiDataType_Float, &npc_anim_framerate.get_value(),
                       &npc_anim_framerate.get_min_value(), &npc_anim_framerate.get_max_value(), "%.2f" );

  auto &npc_ghost_anim_framerate = get_persistent_component<Cmp::Persistent::NpcGhostAnimFramerate>();
  ImGui::SliderScalar( "NPC Animation Ghost Framerate", ImGuiDataType_Float, &npc_ghost_anim_framerate.get_value(),
                       &npc_ghost_anim_framerate.get_min_value(), &npc_ghost_anim_framerate.get_max_value(), "%.2f" );

  auto &npc_damage = get_persistent_component<Cmp::Persistent::NpcDamage>();
  ImGui::SliderScalar( "NPC Damage", ImGuiDataType_S64, &npc_damage.get_value(), &npc_damage.get_min_value(),
                       &npc_damage.get_max_value(), "%.1f" );

  auto &npc_push_back = get_persistent_component<Cmp::Persistent::NpcPushBack>();
  if ( ImGui::SliderScalar( "NPC Push Back Distance", ImGuiDataType_Float, &npc_push_back.get_value(),
                            &npc_push_back.get_min_value(), &npc_push_back.get_max_value(), "%.1f pixels" ) )
  {
  ...
  ImGui::SliderScalar( "NPC Death Animation Framerate", ImGuiDataType_Float, &npc_death_anim_framerate.get_value(),
                       &npc_death_anim_framerate.get_min_value(), &npc_death_anim_framerate.get_max_value(), "%.2f" );

  auto &npc_activate_scale = get_persistent_component<Cmp::Persistent::NpcActivateScale>();
  ImGui::SliderScalar( "NPC Activation Bounding Box Scale Factor", ImGuiDataType_Float, &npc_activate_scale.get_value(),
                       &npc_activate_scale.get_min_value(), &npc_activate_scale.get_max_value(), "%.1f pixels" );
  auto &npc_scan_scale = get_persistent_component<Cmp::Persistent::NpcScanScale>();
  ImGui::SliderScalar( "NPC Scan Bounding Box Scale Factor", ImGuiDataType_Float, &npc_scan_scale.get_value(),
                       &npc_scan_scale.get_min_value(), &npc_scan_scale.get_max_value(), "%.1f pixels" );

  auto &npc_lerp_speed = get_persistent_component<Cmp::Persistent::NpcLerpSpeed>();
  ImGui::SliderScalar( "NPC Speed", ImGuiDataType_Float, &npc_lerp_speed.get_value(), &npc_lerp_speed.get_min_value(),
                       &npc_lerp_speed.get_max_value(), "%.1f" );

  // Audio
  ImGui::SeparatorText( "Audio" );

  auto &music_volume = get_persistent_component<Cmp::Persistent::MusicVolume>();
  ImGui::SliderScalar( "Music Volume", ImGuiDataType_Float, &music_volume.get_value(), &music_volume.get_min_value(),
                       &music_volume.get_max_value(), "%.1f" );

  auto &effects_volume = get_persistent_component<Cmp::Persistent::EffectsVolume>();
  ImGui::SliderScalar( "Effects Volume", ImGuiDataType_Float, &effects_volume.get_value(), &effects_volume.get_min_value(),
                       &effects_volume.get_max_value(), "%.1f" );
