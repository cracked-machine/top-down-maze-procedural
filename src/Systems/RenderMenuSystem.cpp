...
void RenderMenuSystem::render_title()
{
  ...
  getWindow().clear();
  {
    ...
    render_text( "Procedural Maze", 128, { kDisplaySize.x * 0.25f, 100.f }, Alignment::CENTER, 20.f,
                 sf::Color::Black, sf::Color::White );

    render_text( "Press <Enter> key to start", 48, { kDisplaySize.x * 0.25f, 300.f },
                 Alignment::CENTER, 15.f );
...
void RenderMenuSystem::render_settings_widgets( sf::Time deltaTime )
{
  ...
  ImGui::SliderFloat( "Above Water Acceleration Rate", &land_acceleration(), 100.f, 1000.f,
                      "%.1f pixels/second²" );
  auto &land_deceleration = m_reg->ctx().get<Cmp::Persistent::LandDeceleration>();
  ImGui::SliderFloat( "Above Water Deceleration Rate", &land_deceleration(), 100.f, 1000.f,
                      "%.1f pixels/second²" );
  auto &water_acceleration = m_reg->ctx().get<Cmp::Persistent::WaterAcceleration>();
  ImGui::SliderFloat( "Under Water Acceleration Rate", &water_acceleration(), 50.f, 500.f,
                      "%.1f pixels/second²" );
  auto &water_deceleration = m_reg->ctx().get<Cmp::Persistent::WaterDeceleration>();
  ImGui::SliderFloat( "Under Water Deceleration Rate", &water_deceleration(), 50.f, 500.f,
                      "%.1f pixels/second²" );
  ImGui::Separator();
  ...
  ImGui::SliderFloat( "NPC Activation Bounding Box Scale Factor", &npc_activate_scale(), 1.f, 20.f,
                      "%.1f pixels" );
  auto &npc_scan_scale = m_reg->ctx().get<Cmp::Persistent::NPCScanScale>();
  ImGui::SliderFloat( "NPC Scan Bounding Box Scale Factor", &npc_scan_scale(), 1.f, 3.f,
                      "%.1f pixels" );
  auto &pc_detection_scale = m_reg->ctx().get<Cmp::Persistent::PCDetectionScale>();
  ImGui::SliderFloat( "PC Detection Bounding Box Scale Factor", &pc_detection_scale(), 1.f, 20.f,
                      "%.1f pixels" );
