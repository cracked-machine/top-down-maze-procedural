...
void createNPC( entt::registry &registry, entt::entity position_entity, const Sprites::SpriteMetaType &type )
{
  ...
  registry.emplace<Cmp::Position>( new_pos_entity, pos_cmp->position, Sys::BaseSystem::kGridSquareSizePixelsF );
  registry.emplace<Cmp::Armable>( new_pos_entity );
  registry.emplace_or_replace<Cmp::Direction>( new_pos_entity, sf::Vector2f{ 0, 0 } );
  auto &npc_scan_scale = Sys::BaseSystem::get_persistent_component<Cmp::Persistent::NpcScanScale>( registry );
  registry.emplace_or_replace<Cmp::NPCScanBounds>( new_pos_entity, pos_cmp->position, Sys::BaseSystem::kGridSquareSizePixelsF,
...
entt::entity destroyNPC( entt::registry &registry, entt::entity npc_entity )
{
  ...
  else
  {
    ...
    {
      auto npc_pos_cmp_bounds = Cmp::RectBounds( npc_pos_cmp->position, Sys::BaseSystem::kGridSquareSizePixelsF, 1.5f );
      // clang-format off
